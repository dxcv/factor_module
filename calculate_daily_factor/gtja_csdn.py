...
from calculate_daily_factor.gtja_func import *
...
def alpha002(data, dependencies=['closePrice', 'lowestPrice', 'highestPrice'], max_window=2):
    ...
    win_ratio = (2 * data['closePrice'] - data['lowestPrice'] - data['highestPrice']) / (
...
def alpha003(data, dependencies=['closePrice', 'lowestPrice', 'highestPrice'], max_window=6):
    ...
    alpha[condition2] = data['closePrice'][condition2] - np.minimum(
        data['closePrice'][condition2].shift(1).replace(np.NaN, 10000), data['lowestPrice'][condition2])
    alpha[condition3] = data['closePrice'][condition3] - np.maximum(
        data['closePrice'][condition3].shift(1).replace(np.NaN, 0), data['highestPrice'][condition3])
...
def alpha004(data, dependencies=['closePrice', 'turnoverVol'], max_window=20):
    ...
    if MEAN(data['closePrice'], 8) + STD(data['closePrice'], 8) < MEAN(data['closePrice'], 2):
        return -1
    elif MEAN(data['closePrice'], 2) < MEAN(data['closePrice'], 8) - STD(data['closePrice'], 8):
        return 1
    elif 1 <= data['turnoverVol'].iloc[19] / MEAN(data['turnoverVol'], 20):
        return 1
    else:
        return -1
...
def alpha009(data, dependencies=['highestPrice', 'lowestPrice', 'turnoverVol'], max_window=8):
    ...
    part1 = (data['highestPrice'] + data['lowestPrice']) * 0.5 - (
                data['highestPrice'].shift(1) + data['lowestPrice'].shift(1)) * 0.5
...
def alpha011(data, dependencies=['closePrice', 'lowestPrice', 'highestPrice', 'turnoverVol'], max_window=6):
    ...
    # \u8fd16\u5929\u83b7\u5229\u76d8\u6bd4\u4f8b
    return ((2 * data['closePrice'] - data['lowestPrice'] - data['highestPrice']) / (
                data['highestPrice'] - data['lowestPrice']) * data['turnoverVol']).sum(axis=0) * (-1)
...
def alpha016(data, dependencies=['turnoverVol', 'turnoverValue'], max_window=10):
    ...
    # \u611f\u89c9\u5176\u4e2d\u6709\u4e2aTSRANK
    vwap = data['turnoverValue'] / data['turnoverVol']
    corr_vol_vwap = data['turnoverVol'].rank(axis=0, pct=True).rolling(window=5, min_periods=5).corr(
        vwap.rank(axis=0, pct=True))
...
def alpha023(data, dependencies=['closePrice'], max_window=40):
    ...
    alpha = part1.ewm(adjust=False, alpha=float(1) / 20, min_periods=20, ignore_na=False).mean() / (
                part1.ewm(adjust=False, alpha=float(1) / 20, min_periods=20, ignore_na=False).mean() + part2.ewm(
            adjust=False, alpha=float(1) / 20, min_periods=20, ignore_na=False).mean()) * 100
    return alpha.iloc[-1]


def alpha024(data, dependencies=['closePrice'], max_window=10):
    # SMA(CLOSE-DELAY(CLOSE,5),5,1)
    return data['closePrice'].diff(5).ewm(adjust=False, alpha=float(1) / 5, min_periods=5, ignore_na=False).mean().iloc[
        -1]
...
def alpha028(data, dependencies=['KDJ_J'], max_window=13):
    ...
    part2 = data['highestPrice'].rolling(window=9, min_periods=9).max() - data['lowestPrice'].rolling(window=9,
                                                                                                      min_periods=9).min()
    part3 = 3 * SMA(list(part1 / part2 * 100)[-3:], 3, 1)
    part4 = [SMA(list(part1 / part2 * 100)[-5:-2], 3, 1), SMA(list(part1 / part2 * 100)[-4:-1], 3, 1),
             SMA(list(part1 / part2 * 100)[-3:], 3, 1)]
...
def alpha030(data, dependencies=['closePrice', 'PB', 'MktValue'], max_window=81):
    ...
    residual = [idxs.rolling(window=60, min_periods=60).apply(lambda x: multi_var_linregress(x, ret[col], xs)) for col
                in ret.columns]
...
def alpha032(data, dependencies=['highestPrice', 'turnoverVol'], max_window=6):
    ...
    # \u91cf\u4ef7\u9f50\u5347/\u53cd\u8f6c
    part1 = data['highestPrice'].rank(axis=0, pct=True).rolling(window=3, min_periods=3).corr(
        data['turnoverVol'].rank(axis=0, pct=True))
...
def alpha033(data, dependencies=['lowestPrice', 'closePrice', 'turnoverVol'], max_window=241):
    ...
    ret = data['closePrice'].pct_change(periods=1)
    part2 = ((ret.rolling(window=240, min_periods=240).sum() - ret.rolling(window=20,
                                                                           min_periods=20).sum()) / 220).rank(axis=0,
                                                                                                              pct=True)
...
def alpha035(data, dependencies=['openPrice', 'closePrice', 'turnoverVol'], max_window=24):
    ...
    part1 = data['openPrice'].diff(periods=1).rolling(window=15, min_periods=15).apply(lambda x: np.dot(x, w15)).rank(
        axis=0, pct=True)
    part2 = (data['openPrice'] * 0.65 + data['closePrice'] * 0.35).rolling(window=17, min_periods=17).corr(
        data['turnoverVol']).rolling(window=7, min_periods=7).apply(lambda x: np.dot(x, w7)).rank(axis=0, pct=True)
...
def alpha036(data, dependencies=['turnoverValue', 'turnoverVol'], max_window=9):
    ...
    # \u91cf\u4ef7\u9f50\u5347, TSSUM
    vwap = data['turnoverValue'] / data['turnoverVol']
    part1 = data['turnoverVol'].rank(axis=0, pct=True).rolling(window=6, min_periods=6).corr(
        vwap.rank(axis=0, pct=True))
...
def alpha039(data, dependencies=['closePrice', 'openPrice', 'turnoverValue', 'turnoverVol'], max_window=243):
    ...
    part1 = data['closePrice'].diff(periods=2).rolling(window=8, min_periods=8).apply(lambda x: np.dot(x, w8)).rank(
        axis=0, pct=True)
    part2 = parta.rolling(window=14, min_periods=14).corr(partb).rolling(window=12, min_periods=12).apply(
        lambda x: np.dot(x, w12)).rank(axis=0, pct=True)
...
def alpha041(data, dependencies=['turnoverValue', 'turnoverVol'], max_window=9):
    ...
    return \
    (data['turnoverValue'] / data['turnoverVol']).diff(periods=3).rolling(window=5, min_periods=5).max().rank(axis=0,
                                                                                                              pct=True).iloc[
        -1] * (-1)
...
def alpha044(data, dependencies=['turnoverValue', 'turnoverVol', 'lowestPrice'], max_window=29):
    ...
    part1 = (data['turnoverVol'].rolling(window=10, min_periods=10).mean().rolling(window=7, min_periods=7).corr(
        data['lowestPrice'])).rolling(window=6, min_periods=6).apply(lambda x: np.dot(x, w6))
    part1 = part1.iloc[-4:].rank(axis=0, pct=True)
    part2 = (data['turnoverValue'] / data['turnoverVol']).diff(periods=3).rolling(window=10, min_periods=10).apply(
        lambda x: np.dot(x, w10))
...
def alpha045(data, dependencies=['openPrice', 'closePrice', 'turnoverValue', 'turnoverVol'], max_window=165):
    ...
    part2 = ((data['turnoverValue'] / data['turnoverVol']).rolling(window=15, min_periods=15).corr(
        data['turnoverVol'].rolling(window=150, min_periods=150).mean())).rank(axis=0, pct=True)
    return (part1 * part2).iloc[-1]
...
def alpha047(data, dependencies=['closePrice', 'lowestPrice', 'highestPrice'], max_window=15):
    ...
    # RSV\u6280\u672f\u6307\u6807\u53d8\u79cd
    part1 = (data['highestPrice'].rolling(window=6, min_periods=6).max() - data['closePrice']) / (
                data['highestPrice'].rolling(window=6, min_periods=6).max() - data['lowestPrice'].rolling(window=6,
                                                                                                          min_periods=6).min()) * 100
...
def alpha048(data, dependencies=['closePrice', 'turnoverVol'], max_window=20):
    ...
    part2 = data['turnoverVol'].rolling(window=5, min_periods=5).sum() / data['turnoverVol'].rolling(window=20,
                                                                                                     min_periods=20).sum()
    return (part1 * part2).iloc[-1] * (-1)
...
def alpha049(data, dependencies=['highestPrice', 'lowestPrice'], max_window=13):
    ...
    part1[~condition1] = np.maximum(abs(data['highestPrice'].diff(1)[~condition1]),
                                    abs(data['lowestPrice'].diff(1)[~condition1]))
    part2[~condition2] = np.maximum(abs(data['highestPrice'].diff(1)[~condition2]),
                                    abs(data['lowestPrice'].diff(1)[~condition2]))
    alpha = part1.rolling(window=12, min_periods=12).sum() / (
                part1.rolling(window=12, min_periods=12).sum() + part2.rolling(window=12, min_periods=12).sum())
    return alpha.iloc[-1]
...
def alpha054(data, dependencies=['closePrice', 'openPrice'], max_window=10):
    ...
    # \u6ce8\uff0c\u8fd9\u91ccSTD\u6ca1\u6709\u6307\u660e\u5468\u671f
    part1 = abs(data['closePrice'] - data['openPrice']).rolling(window=10, min_periods=10).std() + data['closePrice'] - \
            data['openPrice'] + data['closePrice'].rolling(window=10, min_periods=10).corr(data['openPrice'])
...
def alpha055(data, dependencies=['openPrice', 'lowestPrice', 'closePrice', 'highestPrice'], max_window=21):
    ...
    part2 = abs(data['highestPrice'] - data['closePrice'].shift(1)) + abs(
        data['lowestPrice'] - data['closePrice'].shift(1)) / 2.0 + abs(data['closePrice'] - data['openPrice']).shift(
        1) / 4.0
    condition1 = np.logical_and(abs(data['highestPrice'] - data['closePrice'].shift(1)) > abs(
        data['lowestPrice'] - data['closePrice'].shift(1)),
                                abs(data['highestPrice'] - data['closePrice'].shift(1)) > abs(
                                    data['highestPrice'] - data['lowestPrice'].shift(1)))
    condition2 = np.logical_and(abs(data['lowestPrice'] - data['closePrice'].shift(1)) > abs(
        data['highestPrice'] - data['lowestPrice'].shift(1)),
                                abs(data['lowestPrice'] - data['closePrice'].shift(1)) > abs(
                                    data['highestPrice'] - data['closePrice'].shift(1)))
    part2[~condition1 & condition2] = abs(data['lowestPrice'] - data['closePrice'].shift(1)) + abs(
        data['highestPrice'] - data['closePrice'].shift(1)) / 2.0 + abs(data['closePrice'] - data['openPrice']).shift(
        1) / 4.0
    part2[~condition1 & ~condition2] = abs(data['highestPrice'] - data['lowestPrice'].shift(1)) + abs(
        data['closePrice'] - data['openPrice']).shift(1) / 4.0
    part3 = np.maximum(abs(data['highestPrice'] - data['closePrice'].shift(1)),
                       abs(data['lowestPrice'] - data['closePrice'].shift(1)))
...
def alpha059(data, dependencies=['closePrice', 'lowestPrice', 'highestPrice'], max_window=21):
    ...
    alpha[condition1] = data['closePrice'][condition1] - np.minimum(data['lowestPrice'][condition1],
                                                                    data['closePrice'].shift(1)[condition1])
    alpha[condition2] = data['closePrice'][condition2] - np.maximum(data['highestPrice'][condition2],
                                                                    data['closePrice'].shift(1)[condition2])
    alpha = alpha.rolling(window=20, min_periods=20).sum().iloc[-1]
    return alpha


def alpha060(data, dependencies=['closePrice', 'openPrice', 'lowestPrice', 'highestPrice', 'turnoverVol'],
             max_window=21):
    # SUM((2*CLOSE-LOW-HIGH)./(HIGH-LOW).*VOLUME,20)
    part1 = (2 * data['closePrice'] - data['lowestPrice'] - data['highestPrice']) / (
                data['highestPrice'] - data['lowestPrice']) * data['turnoverVol']
...
def alpha061(data, dependencies=['lowestPrice', 'turnoverValue', 'turnoverVol'], max_window=106):
    ...
    part1 = (data['turnoverValue'] / data['turnoverVol']).diff(periods=1).rolling(window=12, min_periods=12).apply(
        lambda x: np.dot(x, w12)).rank(axis=0, pct=True)
    part2 = (turnover_ma.rolling(window=8, min_periods=8).corr(data['lowestPrice']).rank(axis=0, pct=True)).rolling(
        window=17, min_periods=17).apply(lambda x: np.dot(x, w17)).rank(axis=0, pct=True)
...
def alpha062(data, dependencies=['turnoverVol', 'highestPrice'], max_window=5):
    ...
    return data['turnoverVol'].rank(axis=0, pct=True).rolling(window=5, min_periods=5).corr(data['highestPrice']).iloc[
               -1] * (-1)


def alpha063(data, dependencies=['closePrice'], max_window=7):
    # SMA(MAX(CLOSE-DELAY(CLOSE,1),0),6,1)/SMA(ABS(CLOSE-DELAY(CLOSE,1)),6,1)*100
    part1 = (np.maximum(data['closePrice'].diff(1), 0.0)).ewm(adjust=False, alpha=float(1) / 6, min_periods=0,
...
def alpha064(data, dependencies=['closePrice', 'turnoverValue', 'turnoverVol'], max_window=93):
    ...
    part1 = (vwap.rank(axis=0, pct=True).rolling(window=4, min_periods=4).corr(
        data['turnoverVol'].rank(axis=0, pct=True))).rolling(window=4, min_periods=4).apply(
        lambda x: np.dot(x, w4)).rank(axis=0, pct=True)
    part2 = (data['turnoverVol'].rolling(window=60, min_periods=60).mean().rank(axis=0, pct=True)).rolling(window=4,
                                                                                                           min_periods=4).corr(
        data['closePrice'].rank(axis=0, pct=True))
    part2 = (part2.rolling(window=13, min_periods=13).max()).rolling(window=14, min_periods=14).apply(
        lambda x: np.dot(x, w14)).rank(axis=0, pct=True)
    alpha = np.maximum(part1, part2).iloc[-1] * (-1)
    return alpha
...
def alpha067(data, dependencies=['closePrice'], max_window=25):
    ...
    # RSI24
    part1 = (np.maximum(data['closePrice'].diff(1), 0.0)).ewm(adjust=False, alpha=float(1) / 24, min_periods=0,
                                                              ignore_na=False).mean()
    part2 = (abs(data['closePrice'].diff(1))).ewm(adjust=False, alpha=float(1) / 24, min_periods=0,
                                                  ignore_na=False).mean()
...
def alpha068(data, dependencies=['highestPrice', 'lowestPrice', 'turnoverVol'], max_window=16):
    ...
    part1 = (data['highestPrice'].diff(1) * 0.5 + data['lowestPrice'].diff(1) * 0.5) * (
                data['highestPrice'] - data['lowestPrice']) / data['turnoverVol']
...
def alpha069(data, dependencies=['openPrice', 'highestPrice', 'lowestPrice'], max_window=21):
    ...
    dtm = (data['openPrice'].diff(1) <= 0) * np.maximum(data['highestPrice'] - data['openPrice'],
                                                        data['openPrice'].diff(1))
    dbm = (data['openPrice'].diff(1) >= 0) * np.maximum(data['openPrice'] - data['lowestPrice'],
                                                        data['openPrice'].diff(1))
...
def alpha073(data, dependencies=['turnoverValue', 'turnoverVol', 'closePrice'], max_window=38):
    ...
    part1 = (data['closePrice'].rolling(window=10, min_periods=10).corr(data['turnoverVol'])).rolling(window=16,
                                                                                                      min_periods=16).apply(
        lambda x: np.dot(x, w16))
    part1 = (part1.rolling(window=4, min_periods=4).apply(lambda x: np.dot(x, w4))).rolling(window=5,
                                                                                            min_periods=5).apply(
        lambda x: stats.rankdata(x)[-1] / 5.0)
...
def alpha074(data, dependencies=['lowestPrice', 'turnoverValue', 'turnoverVol'], max_window=68):
    ...
    vwap = data['turnoverValue'] / data['turnoverVol']
    part1 = ((data['lowestPrice'] * 0.35 + vwap * 0.65).rolling(window=20, min_periods=20).sum()).rolling(window=7,
                                                                                                          min_periods=7).corr(
        (data['turnoverVol'].rolling(window=40, min_periods=40).mean()).rolling(window=20, min_periods=20).sum()).rank(
        axis=0, pct=True)
    part2 = (vwap.rank(axis=0, pct=True).rolling(window=6, min_periods=6).corr(
        data['turnoverVol'].rank(axis=0, pct=True))).rank(axis=0, pct=True)
    return (part1 + part2).iloc[-1]
...
def alpha075(data, dependencies=['closePrice', 'openPrice'], max_window=51):
    ...
    print(bm)
    bm_den = pd.DataFrame(data=np.repeat(bm.reshape(len(bm), 1), len(data['closePrice']), axis=0),
                          index=data['closePrice'].index)
    alpha = np.logical_and(data['closePrice'] > data['openPrice'], bm_den).rolling(window=50,
                                                                                   min_periods=50).sum() / bm_den.rolling(
        window=50, min_periods=50).sum()
    return alpha.iloc[-1]
...
def alpha076(data, dependencies=['closePrice', 'turnoverVol'], max_window=21):
    ...
    return (ret_vol.rolling(window=20, min_periods=20).std() / ret_vol.rolling(window=20, min_periods=20).mean()).iloc[
        -1]
...
def alpha077(data, dependencies=['lowestPrice', 'highestPrice', 'turnoverValue', 'turnoverVol'], max_window=50):
    ...
    part1 = (data['highestPrice'] * 0.5 + data['lowestPrice'] * 0.5 - data['turnoverValue'] / data[
        'turnoverVol']).rolling(window=20, min_periods=20).apply(lambda x: np.dot(x, w20)).rank(axis=0, pct=True)
    part2 = ((data['highestPrice'] * 0.5 + data['lowestPrice'] * 0.5).rolling(window=3, min_periods=3).corr(
        data['turnoverVol'].rolling(window=40, min_periods=40).mean())).rolling(window=6, min_periods=6).apply(
        lambda x: np.dot(x, w6)).rank(axis=0, pct=True)
...
def alpha079(data, dependencies=['closePrice', 'openPrice'], max_window=13):
    ...
    # \u5c31\u662fRSI12
    part1 = (np.maximum(data['closePrice'].diff(1), 0.0)).ewm(adjust=False, alpha=float(1) / 12, min_periods=0,
                                                              ignore_na=False).mean()
    part2 = (abs(data['closePrice'].diff(1))).ewm(adjust=False, alpha=float(1) / 12, min_periods=0,
                                                  ignore_na=False).mean()
    return (part1 / part2 * 100).iloc[-1]
...
def alpha085(data, dependencies=['closePrice', 'turnoverVol'], max_window=40):
    ...
    part1 = (data['turnoverVol'] / data['turnoverVol'].rolling(window=20, min_periods=20).mean()).iloc[-20:].rank(
        axis=0, pct=True)
...
def alpha086(data, dependencies=['closePrice'], max_window=21):
    ...
    condition1 = (data['closePrice'].shift(20) * 0.1 + data['closePrice'] * 0.1 - data['closePrice'].shift(
        10) * 0.2) > 0.25
    condition2 = (data['closePrice'].shift(20) * 0.1 + data['closePrice'] * 0.1 - data['closePrice'].shift(
        10) * 0.2) < 0.0
...
def alpha087(data, dependencies=['turnoverValue', 'turnoverVol', 'lowestPrice', 'highestPrice', 'openPrice'],
             max_window=18):
...
def alpha089(data, dependencies=['closePrice'], max_window=37):
    ...
    part1 = data['closePrice'].ewm(adjust=False, alpha=float(2) / 13, min_periods=0, ignore_na=False).mean() - data[
        'closePrice'].ewm(adjust=False, alpha=float(2) / 27, min_periods=0, ignore_na=False).mean()
...
def alpha091(data, dependencies=['closePrice', 'turnoverVol', 'lowestPrice'], max_window=45):
    ...
    part2 = (data['turnoverVol'].rolling(window=40, min_periods=40).mean()).rolling(window=5, min_periods=5).corr(
        data['lowestPrice']).rank(axis=0, pct=True)
    return (part1 * part2).iloc[-1] * (-1)
...
def alpha092(data, dependencies=['closePrice', 'turnoverValue', 'turnoverVol'], max_window=209):
    ...
    part1 = ((data['closePrice'] * 0.35 + data['turnoverValue'] / data['turnoverVol'] * 0.65).diff(2)).rolling(window=3,
                                                                                                               min_periods=3).apply(
        lambda x: np.dot(x, w3)).rank(axis=0, pct=True)
    part2 = abs(
        (data['turnoverVol'].rolling(window=180, min_periods=180).mean()).rolling(window=13, min_periods=13).corr(
            data['closePrice']))
...
def alpha098(data, dependencies=['closePrice'], max_window=201):
    ...
    condition1 = (data['closePrice'].rolling(window=100, min_periods=100).sum() / 100).diff(periods=100) / data[
        'closePrice'].shift(100) <= 0.05
...
def alpha101(data, dependencies=['turnoverValue', 'turnoverVol', 'highestPrice', 'closePrice'], max_window=82):
    ...
    part2 = (part2.rolling(window=11, min_periods=11).corr(data['turnoverVol'].rank(axis=0, pct=True))).rank(axis=0,
                                                                                                             pct=True)
...
def alpha102(data, dependencies=['turnoverVol'], max_window=7):
    ...
    part1 = (np.maximum(data['turnoverVol'].diff(1), 0.0)).ewm(adjust=False, alpha=float(1) / 6, min_periods=0,
                                                               ignore_na=False).mean()
    part2 = abs(data['turnoverVol'].diff(1)).ewm(adjust=False, alpha=float(1) / 6, min_periods=0,
                                                 ignore_na=False).mean()
...
def alpha103(data, dependencies=['lowestPrice'], max_window=20):
    ...
    return (20 - data['lowestPrice'].rolling(window=20, min_periods=20).apply(lambda x: 19 - x.argmin(axis=0))).iloc[
               -1] * 5.0
...
def alpha105(data, dependencies=['openPrice', 'turnoverVol'], max_window=10):
    ...
    alpha = (data['openPrice'].rank(axis=0, pct=True)).rolling(window=10, min_periods=10).corr(
        data['turnoverVol'].rank(axis=0, pct=True))
...
def alpha108(data, dependencies=['highestPrice', 'turnoverValue', 'turnoverVol'], max_window=126):
    ...
    part2 = ((data['turnoverVol'].rolling(window=120, min_periods=120).mean()).rolling(window=6, min_periods=6).corr(
        data['turnoverValue'] / data['turnoverVol'])).rank(axis=0, pct=True)
    return (part1 ** part2).iloc[-1] * (-1)


def alpha109(data, dependencies=['highestPrice', 'lowestPrice'], max_window=20):
    # SMA(HIGH-LOW,10,2)/SMA(SMA(HIGH-LOW,10,2),10,2)
    part1 = (data['highestPrice'] - data['lowestPrice']).ewm(adjust=False, alpha=float(2) / 10, min_periods=0,
                                                             ignore_na=False).mean()
...
def alpha110(data, dependencies=['closePrice', 'highestPrice', 'lowestPrice'], max_window=21):
    ...
    part1 = (np.maximum(data['highestPrice'] - data['closePrice'].shift(1), 0.0)).rolling(window=20,
                                                                                          min_periods=20).sum()
    part2 = (np.maximum(data['closePrice'].shift(1) - data['lowestPrice'], 0.0)).rolling(window=20,
                                                                                         min_periods=20).sum()
...
def alpha111(data, dependencies=['lowestPrice', 'highestPrice', 'closePrice', 'turnoverVol'], max_window=11):
    ...
    win_vol = data['turnoverVol'] * (data['closePrice'] * 2 - data['lowestPrice'] - data['highestPrice']) / (
                data['highestPrice'] - data['lowestPrice'])
    alpha = win_vol.ewm(adjust=False, alpha=float(2) / 11, min_periods=0, ignore_na=False).mean() - win_vol.ewm(
        adjust=False, alpha=float(2) / 4, min_periods=0, ignore_na=False).mean()
    return alpha.iloc[-1]
...
def alpha113(data, dependencies=['closePrice', 'turnoverVol'], max_window=28):
    ...
    part3 = ((data['closePrice'].rolling(window=5, min_periods=5).sum()).rolling(window=2, min_periods=2).corr(
        data['closePrice'].rolling(window=20, min_periods=20).sum())).rank(axis=0, pct=True)
    return (part1 * part2 * part3).iloc[-1] * (-1)


def alpha114(data, dependencies=['highestPrice', 'lowestPrice', 'closePrice', 'turnoverValue', 'turnoverVol'],
             max_window=8):
    # RANK(DELAY((HIGH-LOW)/(SUM(CLOSE,5)/5),2))*RANK(RANK(VOLUME))/((HIGH-LOW)/(SUM(CLOSE,5)/5)/(VWAP-CLOSE))
    # RANK/RANK\u8c8c\u4f3c\u6ca1\u5fc5\u8981
    part1 = ((data['highestPrice'] - data['lowestPrice']) / (
        data['closePrice'].rolling(window=5, min_periods=5).mean())).shift(2).rank(axis=0, pct=True)
    part2 = data['turnoverVol'].rank(axis=0, pct=True).rank(axis=0, pct=True)
    part3 = (data['highestPrice'] - data['lowestPrice']) / (
        data['closePrice'].rolling(window=5, min_periods=5).mean()) / (
                        data['turnoverValue'] / data['turnoverVol'] - data['closePrice'])
...
def alpha115(data, dependencies=['highestPrice', 'lowestPrice', 'turnoverVol', 'closePrice'], max_window=40):
    ...
    part2 = (((data['highestPrice'] * 0.5 + data['lowestPrice'] * 0.5).rolling(window=4, min_periods=4).apply(
        lambda x: stats.rankdata(x)[-1] / 4.0)).rolling(window=7, min_periods=7).corr(
        data['turnoverVol'].rolling(window=10, min_periods=10).apply(lambda x: stats.rankdata(x)[-1] / 10.0))).rank(
        axis=0, pct=True)
    return (part1 ** part2).iloc[-1]
...
def alpha118(data, dependencies=['highestPrice', 'openPrice', 'lowestPrice'], max_window=20):
    ...
    alpha = (data['highestPrice'] - data['openPrice']).rolling(window=20, min_periods=20).sum() / (
                data['openPrice'] - data['lowestPrice']).rolling(window=20, min_periods=20).sum() * 100.0
    return alpha.iloc[-1]
...
def alpha119(data, dependencies=['turnoverValue', 'turnoverVol', 'openPrice'], max_window=62):
    ...
    part1 = (
        (data['turnoverVol'].rolling(window=5, min_periods=5).mean()).rolling(window=26, min_periods=26).sum()).rolling(
        window=5, min_periods=5).corr(data['turnoverValue'] / data['turnoverVol'])
    part1 = (part1.rolling(window=7, min_periods=7).apply(lambda x: np.dot(x, w7))).rank(axis=0, pct=True)
    part2 = ((data['turnoverVol'].rolling(window=15, min_periods=15).mean()).rank(axis=0, pct=True)).rolling(window=21,
                                                                                                             min_periods=21).corr(
        data['openPrice'].rank(axis=0, pct=True))
    part2 = (((part2.rolling(window=9, min_periods=9).min()).rolling(window=7, min_periods=7).apply(
        lambda x: stats.rankdata(x)[-1] / 7.0)).rolling(window=8, min_periods=8).apply(lambda x: np.dot(x, w8))).rank(
        axis=0, pct=True)
    return (part1 - part2).iloc[-1]
...
def alpha121(data, dependencies=['turnoverValue', 'turnoverVol'], max_window=83):
    ...
    part2 = (data['turnoverVol'].rolling(window=60, min_periods=60).mean()).rolling(window=2, min_periods=2).apply(
        lambda x: stats.rankdata(x)[-1] / 2.0)
    part2 = ((vwap.rolling(window=20, min_periods=20).apply(lambda x: stats.rankdata(x)[-1] / 20.0)).rolling(window=18,
                                                                                                             min_periods=18).corr(
        part2)).rolling(window=3, min_periods=3).apply(lambda x: stats.rankdata(x)[-1] / 3.0)
    return (part1 ** part2).iloc[-1] * (-1)
...
def alpha122(data, dependencies=['closePrice'], max_window=40):
    ...
    part1 = (part1.ewm(adjust=False, alpha=float(2) / 13, min_periods=0, ignore_na=False).mean()).ewm(adjust=False,
                                                                                                      alpha=float(
                                                                                                          2) / 13,
                                                                                                      min_periods=0,
                                                                                                      ignore_na=False).mean()
...
def alpha123(data, dependencies=['highestPrice', 'lowestPrice', 'turnoverVol'], max_window=89):
    ...
    part1 = ((data['turnoverVol'].rolling(window=60, min_periods=60).mean()).rolling(window=20,
                                                                                     min_periods=20).sum()).rolling(
        window=9, min_periods=9).corr(part1).rank(axis=0, pct=True)
...
def alpha124(data, dependencies=['closePrice', 'turnoverValue', 'turnoverVol'], max_window=32):
    ...
    part2 = ((data['closePrice'].rolling(window=30, min_periods=30).max()).rank(axis=0, pct=True)).rolling(window=2,
                                                                                                           min_periods=2).apply(
        lambda x: np.dot(x, w2))
    return (part1 / part2).iloc[-1]
...
def alpha125(data, dependencies=['closePrice', 'turnoverValue', 'turnoverVol'], max_window=117):
    ...
    part1 = (data['turnoverVol'].rolling(window=80, min_periods=80).mean()).rolling(window=17, min_periods=17).corr(
        vwap)
    part1 = (part1.rolling(window=20, min_periods=20).apply(lambda x: np.dot(x, w20))).rank(axis=0, pct=True)
    part2 = ((data['closePrice'] * 0.5 + vwap * 0.5).diff(periods=3)).rolling(window=16, min_periods=16).apply(
        lambda x: np.dot(x, w16)).rank(axis=0, pct=True)
    return (part1 / part2).iloc[-1]
...
def alpha127(data, dependencies=['closePrice'], max_window=24):
    ...
    alpha = (data['closePrice'] - data['closePrice'].rolling(window=12, min_periods=12).max()) / data[
        'closePrice'].rolling(window=12, min_periods=12).max() * 100
...
def alpha130(data, dependencies=['lowestPrice', 'highestPrice', 'turnoverVol', 'turnoverValue'], max_window=59):
    ...
    part1 = (data['turnoverVol'].rolling(window=40, min_periods=40).mean()).rolling(window=9, min_periods=9).corr(
        data['highestPrice'] * 0.5 + data['lowestPrice'] * 0.5)
    part1 = part1.rolling(window=10, min_periods=10).apply(lambda x: np.dot(x, w10)).rank(axis=0, pct=True)
    part2 = (data['turnoverVol'].rank(axis=0, pct=True)).rolling(window=7, min_periods=7).corr(
        vwap.rank(axis=0, pct=True))
    part2 = part2.rolling(window=3, min_periods=3).apply(lambda x: np.dot(x, w3)).rank(axis=0, pct=True)
    return (part1 / part2).iloc[-1]
...
def alpha131(data, dependencies=['turnoverValue', 'turnoverVol', 'closePrice'], max_window=86):
    ...
    part2 = (data['turnoverVol'].rolling(window=50, min_periods=50).mean()).rolling(window=18, min_periods=18).corr(
        data['closePrice'])
...
def alpha138(data, dependencies=['lowestPrice', 'turnoverValue', 'turnoverVol'], max_window=126):
    ...
    w20 = np.array(range(1, 21))
    w16 = np.array(range(1, 17))
    part1 = ((data['lowestPrice'] * 0.7 + data['turnoverValue'] / data['turnoverVol'] * 0.3).diff(3)).rolling(window=20,
                                                                                                              min_periods=20).apply(
        lambda x: np.dot(x, w20)).rank(axis=0, pct=True)
    part2 = (data['turnoverVol'].rolling(window=60, min_periods=60).mean()).rolling(window=17, min_periods=17).apply(
        lambda x: stats.rankdata(x)[-1] / 17.0)
    part2 = part2.rolling(window=5, min_periods=5).corr(
        data['lowestPrice'].rolling(window=8, min_periods=8).apply(lambda x: stats.rankdata(x)[-1] / 8.0))
    part2 = ((part2.rolling(window=19, min_periods=19).apply(lambda x: stats.rankdata(x)[-1] / 19.0)).rolling(window=16,
                                                                                                              min_periods=16).apply(
        lambda x: np.dot(x, w16))).rolling(window=7, min_periods=7).apply(lambda x: stats.rankdata(x)[-1] / 7.0)
    return (part1 - part2).iloc[-1] * (-1)
...
def alpha140(data, dependencies=['openPrice', 'lowestPrice', 'highestPrice', 'closePrice', 'turnoverVol'],
             max_window=99):
    ...
    w8 = np.array(range(1, 9))
    w7 = np.array(range(1, 8))
    part1 = data['openPrice'].rank(axis=0, pct=True) + data['lowestPrice'].rank(axis=0, pct=True) - data[
        'highestPrice'].rank(axis=0, pct=True) - data['closePrice'].rank(axis=0, pct=True)
    part1 = part1.rolling(window=8, min_periods=8).apply(lambda x: np.dot(x, w8)).rank(axis=0, pct=True)
    part2 = (data['turnoverVol'].rolling(window=60, min_periods=60).mean()).rolling(window=20, min_periods=20).apply(
        lambda x: stats.rankdata(x)[-1] / 20.0)
    part2 = part2.rolling(window=8, min_periods=8).corr(
        data['closePrice'].rolling(window=8, min_periods=8).apply(lambda x: stats.rankdata(x)[-1] / 8.0))
    part2 = (part2.rolling(window=7, min_periods=7).apply(lambda x: np.dot(x, w7))).rolling(window=3,
                                                                                            min_periods=3).apply(
        lambda x: stats.rankdata(x)[-1] / 3.0)
    return np.minimum(part1, part2).iloc[-1]


def alpha141(data, dependencies=['highestPrice', 'turnoverVol'], max_window=25):
    # (RANK(CORR(RANK(HIGH),RANK(MEAN(VOLUME,15)),9))*-1)
    alpha = ((data['turnoverVol'].rolling(window=15, min_periods=15).mean().rank(axis=0, pct=True)).rolling(window=9,
                                                                                                            min_periods=9).corr(
        data['highestPrice'].rank(axis=0, pct=True))).rank(axis=0, pct=True)
    return alpha.iloc[-1] * (-1)
...
def alpha142(data, dependencies=['closePrice', 'turnoverVol'], max_window=25):
    ...
    part1 = (data['closePrice'].rolling(window=10, min_periods=10).apply(lambda x: stats.rankdata(x)[-1] / 10.0)).rank(
        axis=0, pct=True)
    part2 = (data['closePrice'].diff(1)).diff(1).rank(axis=0, pct=True)
    part3 = (data['turnoverVol'] / data['turnoverVol'].rolling(window=20, min_periods=20).mean()).rolling(window=5,
                                                                                                          min_periods=5).apply(
        lambda x: stats.rankdata(x)[-1] / 5.0).rank(axis=0, pct=True)
    return (part1 * part2 * part3).iloc[-1] * (-1)
...
def alpha145(data, dependencies=['turnoverVol'], max_window=26):
    ...
    alpha = (data['turnoverVol'].rolling(window=9, min_periods=9).mean() - data['turnoverVol'].rolling(window=26,
                                                                                                       min_periods=26).mean()) / \
...
def alpha146(data, dependencies=['closePrice'], max_window=121):
    ...
    # \u5047\u8bbe\u6700\u540e\u4e00\u4e2aSMA(X,60,1)
    sma = (data['closePrice'].pct_change(1)).ewm(adjust=False, alpha=float(2) / 61, min_periods=0,
                                                 ignore_na=False).mean()
...
def alpha151(data, dependencies=['closePrice'], max_window=41):
    ...
    return \
    (data['closePrice'].diff(20)).ewm(adjust=False, alpha=float(1) / 20, min_periods=0, ignore_na=False).mean().iloc[-1]
...
def alpha152(data, dependencies=['closePrice'], max_window=59):
    ...
    part1 = ((data['closePrice'] / data['closePrice'].shift(9)).shift(1)).ewm(adjust=False, alpha=float(1) / 9,
                                                                              min_periods=0,
                                                                              ignore_na=False).mean().shift(1)
    alpha = (part1.rolling(window=12, min_periods=12).mean() - part1.rolling(window=26, min_periods=26).mean()).ewm(
        adjust=False, alpha=float(1) / 9, min_periods=0, ignore_na=False).mean()
    return alpha.iloc[-1]
...
def alpha154(data, dependencies=['turnoverValue', 'turnoverVol'], max_window=198):
    ...
    part2 = (data['turnoverVol'].rolling(window=180, min_periods=180).mean()).rolling(window=18, min_periods=18).corr(
        vwap)
    return (part2 - part1).iloc[-1]
...
def alpha156(data, dependencies=['turnoverValue', 'turnoverVol', 'openPrice', 'lowestPrice'], max_window=9):
    ...
    part1 = ((data['turnoverValue'] / data['turnoverVol']).diff(5)).rolling(window=3, min_periods=3).apply(
        lambda x: np.dot(x, w3))
...
def alpha157(data, dependencies=['closePrice'], max_window=12):
    ...
    part1 = np.log(
        (((data['closePrice'] - 1.0).diff(5).rank(axis=0, pct=True) * (-1)).rank(axis=0, pct=True)).rolling(window=2,
                                                                                                            min_periods=2).min())
...
def alpha161(data, dependencies=['closePrice', 'lowestPrice', 'highestPrice'], max_window=13):
    ...
    part1 = np.maximum(data['highestPrice'] - data['lowestPrice'],
                       abs(data['closePrice'].shift(1) - data['highestPrice']))
...
def alpha162(data, dependencies=['closePrice'], max_window=25):
    ...
    # -MIN(SMA(MAX(CLOSE-DELAY(CLOSE,1),0),12,1)/SMA(ABS(CLOSE-DELAY(CLOSE,1)),12,1)*100,12))
    den = (np.maximum(data['closePrice'].diff(1), 0.0)).ewm(adjust=False, alpha=float(1) / 12, min_periods=0,
                                                            ignore_na=False).mean() / (
              abs(data['closePrice'].diff(1))).ewm(adjust=False, alpha=float(1) / 12, min_periods=0,
                                                   ignore_na=False).mean() * 100.0
    alpha = (den - den.rolling(window=12, min_periods=12).min()) / (
                den.rolling(window=12, min_periods=12).max() - den.rolling(window=12, min_periods=12).min())
    return alpha.iloc[-1]
...
def alpha163(data, dependencies=['turnoverValue', 'turnoverVol', 'closePrice', 'highestPrice'], max_window=20):
    ...
    alpha = data['closePrice'].pct_change(periods=1) * (
        data['turnoverVol'].rolling(window=20, min_periods=20).mean()) * (
                        data['turnoverValue'] / data['turnoverVol']) * (data['highestPrice'] - data['closePrice']) * (
                -1)
    return alpha.iloc[-1]
...
def alpha165(data, dependencies=['closePrice'], max_window=144):
    ...
    part1 = ((data['closePrice'] - data['closePrice'].rolling(window=48, min_periods=48).mean()).rolling(window=48,
                                                                                                         min_periods=48).sum()).rolling(
        window=48, min_periods=48).max()
    part2 = ((data['closePrice'] - data['closePrice'].rolling(window=48, min_periods=48).mean()).rolling(window=48,
                                                                                                         min_periods=48).sum()).rolling(
        window=48, min_periods=48).min()
...
def alpha166(data, dependencies=['closePrice'], max_window=41):
    ...
    part1 = data['closePrice'].pct_change(periods=1) - (
        data['closePrice'].pct_change(periods=1).rolling(window=20, min_periods=20).mean())
    part1 = part1.rolling(window=20, min_periods=20).sum() * ((-20) * 19 ** 1.5)
    part2 = (((data['closePrice'] / data['closePrice'].shift(1)) ** 2).rolling(window=20,
                                                                               min_periods=20).sum() ** 1.5) * 19 * 18
    return (part1 / part2).iloc[-1]
...
def alpha169(data, dependencies=['closePrice'], max_window=48):
    ...
    part1 = (
        data['closePrice'].diff(1).ewm(adjust=False, alpha=float(1) / 9, min_periods=0, ignore_na=False).mean()).shift(
        1)
    part2 = (part1.rolling(window=12, min_periods=12).mean() - part1.rolling(window=26, min_periods=26).mean()).ewm(
        adjust=False, alpha=float(1) / 10, min_periods=0, ignore_na=False).mean()
    return part2.iloc[-1]
...
def alpha170(data, dependencies=['closePrice', 'turnoverVol', 'highestPrice', 'turnoverValue'], max_window=20):
    ...
    vwap = data['turnoverValue'] / data['turnoverVol']
    part1 = (1.0 / data['closePrice']).rank(axis=0, pct=True) * data['turnoverVol'] / (
        data['turnoverVol'].rolling(window=20, min_periods=20).mean())
    part2 = ((data['highestPrice'] - data['closePrice']).rank(axis=0, pct=True) * data['highestPrice']) / (
                data['highestPrice'].rolling(window=5, min_periods=5).sum() / 5.0)
...
def alpha172(data, dependencies=['ADX'], max_window=20):
    ...
    tr = np.maximum(np.maximum(data['highestPrice'] - data['lowestPrice'],
                               (data['highestPrice'] - data['closePrice'].shift(1)).abs()),
                    (data['lowestPrice'] - data['closePrice'].shift(1)).abs())
    part1 = (((ld > 0) & (ld > hd)) * ld).rolling(window=14, min_periods=14).sum() * 100 / tr.rolling(window=14,
                                                                                                      min_periods=14).sum() - (
                        ((hd > 0) & (hd > ld)) * hd).rolling(window=14, min_periods=14).sum() * 100 / tr.rolling(
        window=14, min_periods=14).sum()
    part2 = (((ld > 0) & (ld > hd)) * ld).rolling(window=14, min_periods=14).sum() * 100 / tr.rolling(window=14,
                                                                                                      min_periods=14).sum() + (
                        ((hd > 0) & (hd > ld)) * hd).rolling(window=14, min_periods=14).sum() * 100 / tr.rolling(
        window=14, min_periods=14).sum()
...
def alpha173(data, dependencies=['closePrice'], max_window=39):
    ...
    part3 = (
        (np.log(data['closePrice']).ewm(adjust=False, alpha=float(2) / 13, min_periods=0, ignore_na=False).mean()).ewm(
            adjust=False, alpha=float(2) / 13, min_periods=0, ignore_na=False).mean()).ewm(adjust=False,
                                                                                           alpha=float(2) / 13,
                                                                                           min_periods=0,
                                                                                           ignore_na=False).mean()
    return (part1 - part2 + part3).iloc[-1]
...
def alpha175(data, dependencies=['lowestPrice', 'highestPrice', 'closePrice'], max_window=7):
    ...
    alpha = np.maximum(data['highestPrice'] - data['lowestPrice'],
                       abs(data['closePrice'].shift(1) - data['highestPrice']))
...
def alpha176(data, dependencies=['closePrice', 'highestPrice', 'lowestPrice', 'turnoverVol'], max_window=18):
    ...
    part1 = ((data['closePrice'] - data['lowestPrice'].rolling(window=12, min_periods=12).min()) / (
                data['highestPrice'].rolling(window=12, min_periods=12).max() - data['lowestPrice'].rolling(window=12,
                                                                                                            min_periods=12).min())).rank(
        axis=0, pct=True)
...
def alpha177(data, dependencies=['highestPrice'], max_window=20):
    ...
    return (20 - data['highestPrice'].rolling(window=20, min_periods=20).apply(lambda x: 19 - x.argmax(axis=0))).iloc[
               -1] * 5.0
...
def alpha179(data, dependencies=['lowestPrice', 'turnoverValue', 'turnoverVol'], max_window=62):
    ...
    part1 = (
        (data['turnoverValue'] / data['turnoverVol']).rolling(window=4, min_periods=4).corr(data['turnoverVol'])).rank(
        axis=0, pct=True)
    part2 = (((data['turnoverVol'].rolling(window=50, min_periods=50).mean()).rank(axis=0, pct=True)).rolling(window=12,
                                                                                                              min_periods=12).corr(
        data['lowestPrice'].rank(axis=0, pct=True))).rank(axis=0, pct=True)
    return (part1 * part2).iloc[-1]
...
def alpha180(data, dependencies=['turnoverVol', 'closePrice'], max_window=68):
    ...
    alpha = abs(data['closePrice'].diff(7)).rolling(window=60, min_periods=60).apply(
        lambda x: stats.rankdata(x)[-1] / 60.0) * np.sign(data['closePrice'].diff(7)) * (-1)
...
def alpha181(data, dependencies=['closePrice'], max_window=40):
    ...
    bm_mean = pd.DataFrame(
        data=np.repeat(bm_mean.values.reshape(len(bm_mean.values), 1), len(data['closePrice'].columns), axis=1),
        index=data['closePrice'].index, columns=data['closePrice'].columns)
    ret = data['closePrice'].pct_change(periods=1)
    part1 = (ret - ret.rolling(window=20, min_periods=20).mean() - bm_mean ** 2).rolling(window=20,
                                                                                         min_periods=20).sum()
...
def alpha182(data, dependencies=['closePrice', 'openPrice'], max_window=20):
    ...
    bm = pd.DataFrame(data=np.repeat(bm.values.reshape(len(bm.values), 1), len(data['closePrice'].columns), axis=1),
                      index=data['closePrice'].index, columns=data['closePrice'].columns)
...
def alpha183(data, dependencies=['closePrice'], max_window=72):
    ...
    part1 = ((data['closePrice'] - data['closePrice'].rolling(window=24, min_periods=24).mean()).rolling(window=24,
                                                                                                         min_periods=24).sum()).rolling(
        window=24, min_periods=24).max()
    part2 = ((data['closePrice'] - data['closePrice'].rolling(window=24, min_periods=24).mean()).rolling(window=24,
                                                                                                         min_periods=24).sum()).rolling(
        window=24, min_periods=24).min()
...
def alpha184(data, dependencies=['closePrice', 'openPrice'], max_window=201):
    ...
    part1 = (((data['openPrice'] - data['closePrice']).shift(1)).rolling(window=200, min_periods=200).corr(
        data['closePrice'])).rank(axis=0, pct=True)
...
def alpha188(data, dependencies=['lowestPrice', 'highestPrice'], max_window=11):
    ...
    sma = (data['highestPrice'] - data['lowestPrice']).ewm(adjust=False, alpha=float(2) / 11, min_periods=0,
                                                           ignore_na=False).mean()
...
def alpha189(data, dependencies=['closePrice'], max_window=12):
    ...
    return abs(data['closePrice'] - data['closePrice'].rolling(window=6, min_periods=6).mean()).rolling(window=6,
                                                                                                        min_periods=6).mean().iloc[
        -1]
