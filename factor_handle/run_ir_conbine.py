...
# factor_root_addr=r'G:\work_space\pure_factor_deployment\org_factor'
# factor_dict={}
#
...
#         print(factor_name)
#         factor_rolling_mean_dict[factor_name]=factor_dict[factor_name].rolling(
#             factor_ma_dict[factor_name],min_periods = int(0.6*factor_ma_dict[factor_name])).mean()
#     else:
#         factor_rolling_mean_dict[factor_name]=factor_dict[factor_name]
#     pass

import pickle
# with open('factor_rolling_mean_dict.pkl','wb') as f:
#     pickle.dump(factor_rolling_mean_dict,f)
#     pass

with open('factor_rolling_mean_dict.pkl','rb') as f:
    factor_rolling_mean_dict=pickle.load(f)
    pass
#
# for factor_name in factor_rolling_mean_dict.keys():
#     df=factor_rolling_mean_dict[factor_name]
#     df.index=pd.to_datetime(df.index)
#     df.columns = df.columns.map(lambda x: x[:7] + 'XSHE' if x[-1] == 'Z' else x[:7] + 'XSHG')
#     factor_rolling_mean_dict[factor_name]=df
#     pass
#
#
...
for factor_name in factor_rolling_mean_dict.keys():
    ...
    pass
ir_weight_factor_df.dropna(how='all').to_csv('ir_weight_factor.csv')
pass
